...
app = Flask(__name__)
CORS(app)
print("--- Loading M7_weekly Model and Data ---")

# Define file paths
MODEL_NAME = "A7 files\\M7_weekly.joblib"
DATA_NAME = "A7 files\\A7_weekly_calls.csv"
...
try:
    ...
    print(f"FATAL: Could not load model artifacts. Run train_M7_weekly.py first. Error: {e}")
    MODEL = None

# --- 2. Define Prediction Endpoint ---
@app.route('/predict', methods=['GET'])
def predict():
    """Predicts the next WEEK's call count for A7."""
    ...
    try:
        ...
        # Features from the target_timestamp itself (must match training order)
