# --- Best Model (M2, A7 Daily): XGBoost ---
import pandas as pd
...
print("--- Loading and Preprocessing A7.csv for DAILY analysis ---")
try:
    ...
    df_daily.to_csv('A7_daily_calls.csv')
    print("\nSaved 'A7_daily_calls.csv'")
...
try:
    ...
    plt.title('Daily API Calls for A7 (A7_Daily_calls.png)', fontsize=16)
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Number of Calls', fontsize=12)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig('a7_daily_calls.png')
    print("Saved 'a7_daily_calls.png'")
...
try:
    ...
    if n_lags > 0:
        ...
        ax.set_title('Autocorrelation Function (ACF) for A7 (a7_acf_daily.png)')
        ax.set_xlabel('Lags (days)')
        ax.set_ylabel('ACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a7_acf_daily.png')
        print("Saved 'a7_acf_daily.png'")
        plt.close(fig)

        # PACF Plot
        fig, ax = plt.subplots(figsize=(12, 5))
        plot_pacf(df_daily['call_count'], lags=n_lags, ax=ax)
        ax.set_title('Partial Autocorrelation Function (PACF) for A7 (a7_pacf_daily.png)')
        ax.set_xlabel('Lags (days)')
        ax.set_ylabel('PACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a7_pacf_daily.png')
        print("Saved 'a7_pacf_daily.png'")
        plt.close(fig)
    else:
...
print("\n\n--- M2 Model Competition (A7 Daily Data) ---")
...
try:
    df = pd.read_csv('A7_daily_calls.csv', index_col='datetime', parse_dates=True)
    df = df.asfreq('D') # Ensure daily frequency
    print(f"Loaded 'A7_daily_calls.csv'. Total days: {len(df)}")
except Exception as e:
    print(f"Error loading data: {e}")
    print("Please make sure 'A7_daily_calls.csv' exists (run the EDA script first).")
    raise
...
print("\n--- M2 Model Competition Results (A7 Daily) ---")
...
if perf_df.empty:
    ...
    print(f"\n--- Best Model (M2, A7 Daily): {best_model_name} ---")
...
# Save predictions
predictions_df.to_csv('A7_M2_predictions_DAILY.csv')
print("\nSaved all model predictions to 'A7_M2_predictions_DAILY.csv'")
...
try:
    ...
    plt.title(f'M2 (A7, Daily) - Best Model Forecast ({best_model_name}) vs. Actual', fontsize=16)
    plt.xlabel('Date')
    plt.ylabel('Number of Calls')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('m2_A7_best_model_forecast_DAILY.png')
    print("Saved 'm2_A7_best_model_forecast_DAILY.png'")
    plt.close()

except Exception as e:
    print(f"Error generating plot: {e}")

print("\n--- M2 Full Competition Analysis (A7 Daily) Complete. ---")