...
print("--- Loading and Preprocessing A7.csv for MONTHLY analysis ---")
try:
    ...
    df_monthly.to_csv('A7_monthly_calls.csv')
    print("\nSaved 'A7_monthly_calls.csv'")
...
try:
    ...
    plt.title('Monthly API Calls for A7 (A7_Monthly_calls.png)', fontsize=16)
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Number of Calls', fontsize=12)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig('a7_monthly_calls.png')
    print("Saved 'a7_monthly_calls.png'")
...
try:
    ...
    if n_lags > 0:
        ...
        ax.set_title('Autocorrelation Function (ACF) for A7 (a7_acf_monthly.png)')
        ax.set_xlabel('Lags (months)')
        ax.set_ylabel('ACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a7_acf_monthly.png')
        print("Saved 'a7_acf_monthly.png'")
        plt.close(fig)

        # PACF Plot
        fig, ax = plt.subplots(figsize=(12, 5))
        plot_pacf(df_monthly['call_count'], lags=n_lags, ax=ax)
        ax.set_title('Partial Autocorrelation Function (PACF) for A7 (a7_pacf_monthly.png)')
        ax.set_xlabel('Lags (months)')
        ax.set_ylabel('PACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a7_pacf_monthly.png')
        print("Saved 'a7_pacf_monthly.png'")
        plt.close(fig)
    else:
...
print("\n\n--- M2 Model Competition (A7 Monthly Data) ---")
...
try:
    df = pd.read_csv('A7_monthly_calls.csv', index_col='datetime', parse_dates=True)
    df = df.asfreq('MS') # Ensure monthly frequency
    print(f"Loaded 'A7_monthly_calls.csv'. Total months: {len(df)}")
except Exception as e:
    print(f"Error loading data: {e}")
    print("Please make sure 'A7_monthly_calls.csv' exists (run the EDA script first).")
    raise
...
print("\n--- M2 Model Competition Results (A7 Monthly) ---")
...
if perf_df.empty:
    ...
    print(f"\n--- Best Model (M2, A7 Monthly): {best_model_name} ---")
...
# Save predictions
predictions_df.to_csv('A7_M2_predictions_MONTHLY.csv')
print("\nSaved all model predictions to 'A7_M2_predictions_MONTHLY.csv'")
...
try:
    ...
    plt.title(f'M2 (A7, Monthly) - Best Model Forecast ({best_model_name}) vs. Actual', fontsize=16)
    plt.xlabel('Date')
    plt.ylabel('Number of Calls')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('m2_A7_best_model_forecast_MONTHLY.png')
    print("Saved 'm2_A7_best_model_forecast_MONTHLY.png'")
    plt.close()

except Exception as e:
    print(f"Error generating plot: {e}")

print("\n--- M2 Full Competition Analysis (A7 Monthly) Complete. ---")