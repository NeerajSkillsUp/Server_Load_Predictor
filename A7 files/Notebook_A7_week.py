# --- Best Model (M2, A7 Weekly): XGBoost ---
import pandas as pd
...
print("--- Loading and Preprocessing A7.csv for WEEKLY analysis ---")
try:
    ...
    df_weekly.to_csv('A7_weekly_calls.csv')
    print("\nSaved 'A7_weekly_calls.csv'")
...
try:
    ...
    plt.title('Weekly API Calls for A7 (A7_Weekly_calls.png)', fontsize=16)
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Number of Calls', fontsize=12)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig('a7_weekly_calls.png')
    print("Saved 'a7_weekly_calls.png'")
...
try:
    ...
    if n_lags > 0:
        ...
        ax.set_title('Autocorrelation Function (ACF) for A7 (a7_acf_weekly.png)')
        ax.set_xlabel('Lags (weeks)')
        ax.set_ylabel('ACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a7_acf_weekly.png')
        print("Saved 'a7_acf_weekly.png'")
        plt.close(fig)

        # PACF Plot
        fig, ax = plt.subplots(figsize=(12, 5))
        plot_pacf(df_weekly['call_count'], lags=n_lags, ax=ax)
        ax.set_title('Partial Autocorrelation Function (PACF) for A7 (a7_pacf_weekly.png)')
        ax.set_xlabel('Lags (weeks)')
        ax.set_ylabel('PACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a7_pacf_weekly.png')
        print("Saved 'a7_pacf_weekly.png'")
        plt.close(fig)
    else:
...
print("\n\n--- M2 Model Competition (A7 Weekly Data) ---")
...
try:
    df = pd.read_csv('A7_weekly_calls.csv', index_col='datetime', parse_dates=True)
    df = df.asfreq('W') # Ensure weekly frequency
    print(f"Loaded 'A7_weekly_calls.csv'. Total weeks: {len(df)}")
except Exception as e:
    print(f"Error loading data: {e}")
    print("Please make sure 'A7_weekly_calls.csv' exists (run the EDA script first).")
    raise
...
print("\n--- M2 Model Competition Results (A7 Weekly) ---")
...
if perf_df.empty:
    ...
    print(f"\n--- Best Model (M2, A7 Weekly): {best_model_name} ---")
...
# Save predictions
predictions_df.to_csv('A7_M2_predictions_WEEKLY.csv')
print("\nSaved all model predictions to 'A7_M2_predictions_WEEKLY.csv'")
...
try:
    ...
    plt.title(f'M2 (A7, Weekly) - Best Model Forecast ({best_model_name}) vs. Actual', fontsize=16)
    plt.xlabel('Date')
    plt.ylabel('Number of Calls')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('m2_A7_best_model_forecast_WEEKLY.png')
    print("Saved 'm2_A7_best_model_forecast_WEEKLY.png'")
    plt.close()

except Exception as e:
    print(f"Error generating plot: {e}")

print("\n--- M2 Full Competition Analysis (A7 Weekly) Complete. ---")