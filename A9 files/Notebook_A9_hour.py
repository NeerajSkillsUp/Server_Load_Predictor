# --- Best Model (M2, A9 Hourly): Poisson GLM ---
import pandas as pd
...
print("--- Loading and Preprocessing A9.csv ---")
try:
    ...
    df_hourly.to_csv('A9_hourly_calls.csv')
    print("\nSaved 'A9_hourly_calls.csv'")
...
try:
    ...
    plt.title('Hourly API Calls for A9 (A9_hourly_calls.png)', fontsize=16)
    plt.xlabel('Time', fontsize=12)
    plt.ylabel('Number of Calls', fontsize=12)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig('a9_hourly_calls.png')
    print("Saved 'a9_hourly_calls.png'")
...
try:
    ...
    if n_lags > 0:
        ...
        ax.set_title('Autocorrelation Function (ACF) for A9 (a9_acf.png)')
        ax.set_xlabel('Lags (hours)')
        ax.set_ylabel('ACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a9_acf.png')
        print("Saved 'a9_acf.png'")
        plt.close(fig)

        # PACF Plot
        fig, ax = plt.subplots(figsize=(12, 5))
        plot_pacf(df_hourly['call_count'], lags=n_lags, ax=ax)
        ax.set_title('Partial Autocorrelation Function (PACF) for A9 (a9_pacf.png)')
        ax.set_xlabel('Lags (hours)')
        ax.set_ylabel('PACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a9_pacf.png')
        print("Saved 'a9_pacf.png'")
        plt.close(fig)
    else:
...
print("\n\n--- M2 Model Competition (A9 Hourly Data) ---")
...
try:
    df = pd.read_csv('A9_hourly_calls.csv', index_col='datetime', parse_dates=True)
    df = df.asfreq('H') # Ensure hourly frequency
    print(f"Loaded 'A9_hourly_calls.csv'. Total hours: {len(df)}")
except Exception as e:
    print(f"Error loading data: {e}")
    print("Please make sure 'A9_hourly_calls.csv' exists (run the EDA script first).")
    raise
...
print("\n--- M2 Model Competition Results (A9 Hourly) ---")
...
if perf_df.empty:
    ...
    print(f"\n--- Best Model (M2, A9 Hourly): {best_model_name} ---")
...
# Save predictions
predictions_df.to_csv('A9_M2_predictions_FULL.csv')
print("\nSaved all model predictions to 'A9_M2_predictions_FULL.csv'")

# --- 11. Plot Best Model Forecast ---
print("Generating forecast plot for best model (A9 Hourly)...")
try:
    plt.figure(figsize=(18, 7))
    ...
    plt.title(f'M2 (A9) - Best Model Forecast ({best_model_name}) vs. Actual', fontsize=16)
    plt.xlabel('Time')
    plt.ylabel('Number of Calls')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('m2_A9_best_model_forecast_FULL.png')
    print("Saved 'm2_A9_best_model_forecast_FULL.png'")
    plt.close()

except Exception as e:
    print(f"Error generating plot: {e}")

print("\n--- M2 Full Competition Analysis (A9 Hourly) Complete. ---")