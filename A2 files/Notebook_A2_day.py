...
print("--- Loading and Preprocessing A2.csv for DAILY analysis ---")
try:
    ...
    # *** CORRECTION: Changed filename to _daily_ ***
    df_daily.to_csv('A2_daily_calls.csv')
    print("\nSaved 'A2_daily_calls.csv'")
...
try:
    ...
    plt.title('Daily API Calls for A2 (A2_Daily_calls.png)', fontsize=16)
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Number of Calls', fontsize=12)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig('a2_daily_calls.png')
    print("Saved 'a2_daily_calls.png'")
...
try:
    ...
    if n_lags > 0:
        ...
        ax.set_title('Autocorrelation Function (ACF) for A2 (a2_acf_daily.png)')
        ax.set_xlabel('Lags (days)')
        ax.set_ylabel('ACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a2_acf_daily.png')
        print("Saved 'a2_acf_daily.png'")
        plt.close(fig)

        # PACF Plot
        fig, ax = plt.subplots(figsize=(12, 5))
        plot_pacf(df_daily['call_count'], lags=n_lags, ax=ax)
        ax.set_title('Partial Autocorrelation Function (PACF) for A2 (a2_pacf_daily.png)')
        ax.set_xlabel('Lags (days)')
        ax.set_ylabel('PACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a2_pacf_daily.png')
        print("Saved 'a2_pacf_daily.png'")
        plt.close(fig)
    else:
...
print("\n\n--- M2 Model Competition (A2 Daily Data) ---")

# --- 0. Function Definition ---
# *** CORRECTION: Updated features for DAILY data (weekly seasonality) ***
def create_features(df, target_col):
    """Create time-based and lag features for DAILY data."""
    df_feat = df.copy()
    # df_feat['hour'] is irrelevant for daily data
...
try:
    # *** CORRECTION: Loading the correct daily CSV ***
    df = pd.read_csv('A2_daily_calls.csv', index_col='datetime', parse_dates=True)
    df = df.asfreq('D') # Ensure daily frequency
    print(f"Loaded 'A2_daily_calls.csv'. Total days: {len(df)}")
except Exception as e:
    print(f"Error loading data: {e}")
    print("Please make sure 'A2_daily_calls.csv' exists (run the EDA script first).")
    raise

# --- 2. Train/Test Split ---
# *** CORRECTION: Using test_size_days, not hours ***
...
# *** CORRECTION: Using s=7 (weekly) seasonality ***
...
try:
    # *** CORRECTION: Using seasonal_order m=7 (weekly) ***
...
predictions_df.to_csv('A2_M2_predictions_DAILY.csv')
print("\nSaved all model predictions to 'A2_M2_predictions_DAILY.csv'")
...
try:
    ...
    best_pred = None
    if best_model_name == 'Seasonal Naive (s=7)':
        best_pred = y_pred_naive
    elif best_model_name == 'SARIMA (m=7)':
    ...
    plt.title(f'M2 (A2, Daily) - Best Model Forecast ({best_model_name}) vs. Actual', fontsize=16)
    # *** CORRECTION: Updated label ***
    plt.xlabel('Date')
    ...
    plt.tight_layout()
    # *** CORRECTION: Updated filename ***
    plt.savefig('m2_best_model_forecast_DAILY.png')
    print("Saved 'm2_best_model_forecast_DAILY.png'")
    plt.close()

except Exception as e:
    print(f"Error generating plot: {e}")

print("\n--- M2 Full Competition Analysis (Daily) Complete. ---")