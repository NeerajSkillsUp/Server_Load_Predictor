# --- Best Model (M2, Weekly): RandomForest ---
import pandas as pd
...
print("--- Loading and Preprocessing A2.csv for WEEKLY analysis ---")
try:
    ...
    df_weekly.to_csv('A2_weekly_calls.csv')
    print("\nSaved 'A2_weekly_calls.csv'")
...
try:
    ...
    plt.title('Weekly API Calls for A2 (A2_Weekly_calls.png)', fontsize=16)
    plt.xlabel('Date', fontsize=12)
    plt.ylabel('Number of Calls', fontsize=12)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig('a2_weekly_calls.png')
    print("Saved 'a2_weekly_calls.png'")
...
try:
    ...
    if n_lags > 0:
        ...
        ax.set_title('Autocorrelation Function (ACF) for A2 (a2_acf_weekly.png)')
        ax.set_xlabel('Lags (weeks)')
        ax.set_ylabel('ACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a2_acf_weekly.png')
        print("Saved 'a2_acf_weekly.png'")
        plt.close(fig)

        # PACF Plot
        fig, ax = plt.subplots(figsize=(12, 5))
        plot_pacf(df_weekly['call_count'], lags=n_lags, ax=ax)
        ax.set_title('Partial Autocorrelation Function (PACF) for A2 (a2_pacf_weekly.png)')
        ax.set_xlabel('Lags (weeks)')
        ax.set_ylabel('PACF')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('a2_pacf_weekly.png')
        print("Saved 'a2_pacf_weekly.png'")
        plt.close(fig)
    else:
...
print("\n\n--- M2 Model Competition (A2 Weekly Data) ---")

# --- 0. Function Definition ---
# *** CORRECTION: Updated features for WEEKLY data (monthly/yearly seasonality) ***
def create_features(df, target_col):
    """Create time-based and lag features for WEEKLY data."""
    df_feat = df.copy()
    # dayofweek is irrelevant
...
try:
    # *** CORRECTION: Loading the correct weekly CSV ***
    df = pd.read_csv('A2_weekly_calls.csv', index_col='datetime', parse_dates=True)
    df = df.asfreq('W') # Ensure weekly frequency
    print(f"Loaded 'A2_weekly_calls.csv'. Total weeks: {len(df)}")
except Exception as e:
    print(f"Error loading data: {e}")
    print("Please make sure 'A2_weekly_calls.csv' exists (run the EDA script first).")
    raise

# --- 2. Train/Test Split ---
# *** CORRECTION: Using test_size_weeks ***
...
MIN_TRAIN_LEN = 52 + test_size_weeks + 1
if len(df) < MIN_TRAIN_LEN:
     print(f"Warning: Data is very short ({len(df)} weeks).")
     ...
     if len(df) > 53:
         test_size_weeks = max(4, len(df) - 53)
         ...
         # A fallback 's' would be needed, e.g. s=4, but we'll try 52
...
# *** CORRECTION: Using s=52 (yearly) seasonality ***
...
try:
    # *** CORRECTION: Using seasonal_order m=52 (yearly) ***
...
try:
    ...
    # *** CORRECTION: weekly_seasonality=False (it's the base freq), yearly=True
    ...
    # *** CORRECTION: Using freq='W' (weekly) ***
...
print("\n--- M2 Model Competition Results (Weekly) ---")
perf_df = pd.DataFrame(model_performance).T.sort_values(by='MAE')
print(perf_df)

best_model_name = perf_df.index[0]
print(f"\n--- Best Model (M2, Weekly): {best_model_name} ---")

# Store predictions of all models for analysis
# Use y_test_feat for ML models as it's aligned
predictions_df = pd.DataFrame(y_test_feat)
predictions_df.rename(columns={'call_count': 'Actual'}, inplace=True)
try:
    # Naive and SARIMA used test_df
    predictions_df['Seasonal_Naive_Pred'] = y_pred_naive.loc[y_test_feat.index]
    predictions_df['SARIMA_Pred'] = y_pred_sarima.loc[y_test_feat.index]
...
predictions_df.to_csv('A2_M2_predictions_WEEKLY.csv')
print("\nSaved all model predictions to 'A2_M2_predictions_WEEKLY.csv'")

# --- 11. Plot Best Model Forecast ---
print("Generating forecast plot for best model (Weekly)...")
try:
    plt.figure(figsize=(18, 7))
    # *** CORRECTION: Plot last 52 weeks (1 year) of history ***
    ...
    plt.title(f'M2 (A2, Weekly) - Best Model Forecast ({best_model_name}) vs. Actual', fontsize=16)
    plt.xlabel('Date')
    ...
    plt.tight_layout()
    # *** CORRECTION: Updated filename ***
    plt.savefig('m2_best_model_forecast_WEEKLY.png')
    print("Saved 'm2_best_model_forecast_WEEKLY.png'")
    plt.close()

except Exception as e:
    print(f"Error generating plot: {e}")

print("\n--- M2 Full Competition Analysis (Weekly) Complete. ---")