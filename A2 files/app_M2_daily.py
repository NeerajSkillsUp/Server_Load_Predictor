...
app = Flask(__name__)
CORS(app)
print("--- Loading M2_daily Model and Data ---")

MODEL_NAME = "A2 files\\M2_daily.joblib"
DATA_NAME = "A2 files\\A2_daily_calls.csv"
...
try:
    ...
    print(f"FATAL: Could not load model artifacts. Run train_M2_daily.py first. Error: {e}")
    MODEL = None

# --- 2. Define Prediction Endpoint ---
@app.route('/predict', methods=['GET'])
def predict():
    """Predicts the next DAY's call count for A2."""
    ...
    try:
        ...
        # We need data from 7 days ago to calculate lag_7 and rolling_mean_7
        ...
        features_dict = {
            ...
            'lag_7': int(features[4]), # <-- Cast numpy.int64 to int
            'rolling_mean_7': float(features[5]) # <-- Cast numpy.float64 to float
        }
        # --- END FIX ---
        ...
        return jsonify({
            ...
            "predicted_call_count": final_prediction, # This was already cast to int
